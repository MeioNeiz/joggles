...
| **32** | **A failed commit stops pretending, and residency knows its type** | `packages/app/src/deliver.ts`, `core/src/budget.ts`, `core/src/playlist.ts` | **track 26 closed** (it owns `deliver.ts`) | Two defects found by tracks that did not own the file to fix them, both now load-bearing under the redesign. **`deliver()` sends `MODE` after a commit the device answered `ERROR` to**: `session.save()` reports `saved` for any acknowledged `DATCP` because the erases were spent either way, and `deliver()` withholds `MODE` only on `refused`, so an `ERROR` switches the panel to the saved store, shows whatever was there before, and reports `showing: true`, which is the exact outcome its own comment says the guard exists to prevent. Found by track 13 and pinned as current behaviour in `effects-ui/deliver-wide.test.ts` with a note to invert the assertion. **`budget.SaveRecord` carries no DATS type**, so `residentHash()` trusts the last record of *any* type: track 18 raised it, and the redesign now leans on it twice, for the library's "holding" line and for `one-tap`'s free `return` route. A missing type on an old record must read as **unknown**, never as type 1. **Plus the third thing in `session.ts`, moved here from track 28 so one track owns that file**: a progress option on `SaveOpts`, added exactly the way `cancel` was (additive, optional, no signature and no union changed), reporting blocks sent against blocks total, because five silent seconds is what *"it seems to keep having to send the animation to the device"* feels like. The bar that consumes it is track 34. Suite green |
...
| the reviews | `review-32` through `review-38`, and they are the work |
...
- **Never write to the `fd00` OTA service.** It bricked a unit on 2026-08-08. How that is
  enforced is in `CLAUDE.md`; the prohibition is repeated here because it is the one
  mistake that cost hardware.
- **`bun run test` must be green when you stop.** 731 tests as of 2026-08-12; the number
